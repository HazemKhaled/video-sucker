...
export async function downloadInstagramMediaMethod1(
  ...
  try {
    ...
    const additionalDataMatch = html.match(
      /window\.__additionalDataLoaded\s*\(\s*['"](feed|reel)['"]\s*,\s*(\{.+?\})\);<\/script>/,
    );

    // New data format that Instagram uses in 2025
    const reelDataMatch =
      html.match(/window\.__REELDATA__\s*=\s*(\{.+?\});<\/script>/) ||
    ...
    if (!videoUrl && additionalDataMatch && additionalDataMatch[2]) {
      try {
        ...
        if (media) {
          ...
          username =
            media.user?.username || media.owner?.username || "unknown_user";
    ...
    if (!videoUrl) {
      const metaVideoMatch = html.match(
