...
describe("Instagram Downloader Integration", () => {
  it("should process a reel URL through the entire pipeline", () => {
    ...
    const reelUrl =
      "https://instagram.com/reel/CodExampleID?igshid=tracking123";
    ...
    assert.ok(
      ...
      "URL should be normalized to include www",
    );
    assert.ok(
      normalizedUrl.includes("/reel/"),
      "URL should maintain the reel path",
    );
    assert.ok(
    ...
    assert.strictEqual(
      contentType,
      "reel",
      "Content type should be identified as reel",
    );
    ...
    assert.strictEqual(
      reelId,
      "CodExampleID",
      "Reel ID should be extracted correctly",
    );
