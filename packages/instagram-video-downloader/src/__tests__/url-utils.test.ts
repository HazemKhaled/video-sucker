...
describe("Instagram URL Utilities", () => {
  it("should normalize Instagram reel URLs", () => {
    ...
    const expectedFormat =
      /^https:\/\/www\.instagram\.com\/reel(?:s)?\/CodExampleID\/$/;
  ...
  it("should integrate with the parser to detect reel content types", () => {
    ...
    assert.strictEqual(
      getContentType(reelUrl),
      "reel",
      "Reel URL should be identified as reel",
    );
    assert.strictEqual(
      getContentType(reelsUrl),
      "reel",
      "Reels URL should be identified as reel",
    );
    assert.strictEqual(
      getContentType(unknownUrl),
      "unknown",
      "Non-Instagram URL should be unknown",
    );
  });
});
