...
export async function downloadTikTokVideoMethod1(
  ...
  const outputDir = options.outputDir
    ? path.join(options.outputDir, videoId)
    : path.join(process.cwd(), "tiktok", videoId);
  ...
  return {
    ...
    username:
      tikApiData.data.author?.nickname ||
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        throw new Error(
          "Failed to download TikTok video after trying all methods",
        );
      }
    }
  }
