...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!url || !reelPattern.test(url)) {
      return NextResponse.json(
        { 
          ...
          message: 'Please provide a valid Instagram reel URL. Other content types are not supported.'
        },
        { status: 400 }
      );
    }
    ...
    try {
      ...
      if (!result || !result.mediaItems || result.mediaItems.length === 0) {
        return NextResponse.json(
          { 
            ...
            message: 'Could not find any downloadable media in the provided Instagram URL.' 
          },
          { status: 404 }
        );
      }
      ...
      let errorMessage = 'Instagram has strong protections against downloading media. Try another URL or try again later.';
      ...
      if (error instanceof Error) {
        ...
        if (error.message.includes('fetch') || error.message.includes('network')) {
          errorMessage = 'Network error when connecting to Instagram. The service might be temporarily unavailable.';
        } else if (error.message.includes('timeout')) {
          errorMessage = 'The request to Instagram timed out. Try again later when Instagram servers are less busy.';
        } else if (error.message.includes('403') || error.message.includes('forbidden')) {
          errorMessage = 'Access to Instagram content was denied. Instagram may have detected automated access.';
        } else if (error.message.includes('404') || error.message.includes('not found')) {
          errorMessage = 'The Instagram content could not be found. It may have been deleted or made private.';
        }
      }
